...
key_image["img"].fuzz = key_image["img"].quantum_range * 0.50 #threshold %
...
while len(image_names) > 0:
    ...
    if key_image["img"].compare(analyze_me["img"], metric="absolute")[1] > 0.0: #if they're not the same picture
        #put key_image in unique; make analyze_me the new key_image
        unique.append(key_image["name"])
        key_image = analyze_me
        key_image["img"].fuzz = key_image["img"].quantum_range * 0.50 #fuzziness threshold
unique.append(key_image["name"])
